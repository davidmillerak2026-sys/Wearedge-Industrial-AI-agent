...
| Jetson / IPC stdlib fallback | Dependency-light HTTP benchmark for offline edge nodes without FastAPI/Uvicorn. | 历史备用证据：300 iterations / 4500 samples，p95 2ms，max 5ms，Linux `5.15.148-tegra` / `aarch64`，并带 `tegrastats`。当前已被 FastAPI 证据升级替代。 |
...
- FastAPI 远程隔离目录：`/home/ryn/Wearedge-Industrial-AI-agent-fastapi-competition`；该目录内 `.venv` 已具备 FastAPI/Uvicorn 运行环境。
...
- 忽略目录证据：`submission-assets/live-evidence/edge-runtime/06-http-resource-benchmark.*`、`07-edge-runtime-evidence-manifest.md`、`08-tegrastats-http-resource-benchmark.log`、`09-jetson-edge-evidence-collection-summary.json`。
...
- 当前指标：300 iterations / 4500 HTTP samples，p95 6ms，max 33ms，RSS max 32.33MB，Linux `5.15.148-tegra` / `aarch64`，并带 `tegrastats`。
- FastAPI 升级路径说明：Jetson 本机当前无法解析公网 pip 源；本次刷新复用独立 competition FastAPI 目录内已有 `.venv`，没有污染旧项目和系统 Python。
...
```powershell
$env:JETSON_SSH_PASSWORD = "<set locally, do not commit>"
python scripts/collect_jetson_edge_evidence.py --host wearedge-pro.local --user ryn --remote-dir /home/ryn/Wearedge-Industrial-AI-agent-fastapi-competition --iterations 300 --skip-deploy
Remove-Item Env:\JETSON_SSH_PASSWORD
```

If repository code changes, sync the checkout into the FastAPI competition directory while preserving its `.venv`, then rerun the command above. Do not redeploy by deleting the FastAPI directory unless the venv can be recreated offline.
...
| Python environment | `~/Wearedge-Industrial-AI-agent-fastapi-competition/.venv` for FastAPI evidence; isolated competition `.venv` or system `python3` for fallback. |
| Pulled evidence | `submission-assets/live-evidence/edge-runtime/` on this workstation for FastAPI evidence. |
