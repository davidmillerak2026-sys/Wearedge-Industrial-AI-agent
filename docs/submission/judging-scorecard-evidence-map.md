...
| Wearedge integrates with Gongyi Mofang WFC. | WFC project screenshots, Python block saved as `CallWearedgeDecisionApi`, data-table fields, WFC resource package zip. | Dashboard, `ok=true` run log, and HumanApprovalGate must be described as fallback until live replacement is captured. |
