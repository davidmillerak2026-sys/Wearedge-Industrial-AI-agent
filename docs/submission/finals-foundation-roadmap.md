...
| 指标评估闭环 | 决赛明确要求延迟 <=500ms、决策准确率 >=90%。 | 初赛离线报告和 15 条决赛验证集均通过；决赛集五个主方向各 3 条；`docs/finals-latency-benchmark-report.md` 已给出本地回放延迟；`docs/finals-local-gateway-latency-benchmark-report.md` 已给出本地 FastAPI HTTP 网关延迟和资源采样；`docs/finals-jetson-gateway-latency-benchmark-report.md` 已给出 Jetson FastAPI HTTP 网关 300 样本 p95/max 6/8ms。 | 继续扩大到真实/半真实标签集、真实平台 smoke，并区分模型推理延迟、API 往返延迟与规则决策延迟。 |
| 工易魔方执行主干 | 决赛要在 Xcelerator 或工易魔方中完成端到端/工作流验证。 | 资源块、Python Function Block、runbook、Xcelerator OpenAPI 草稿已具备；WFC run log `ok=true` 已有 live 原生日志，Dashboard 与 HumanApprovalGate 仍是 fallback。 | 真实 WFC 工作流跑通：数据表更新、Dashboard、run log ok=true、HumanApprovalGate。 |
...
| 端侧运行证据 | Wearedge 的差异化是端侧 Agent Runtime，而不是云端 Chatbot。 | `docs/edge-agent-runtime-for-xcelerator.md`、runtime profile、Jetson FastAPI latency/resource/tegrastats 证据已具备；stdlib gateway 保留为 fallback。 | 继续采集 IPC / 工控机 latency、断网可用、数据不出厂证据。 |
| 证据边界和审计 | 决赛答辩必须可信，不能把模拟说成真实。 | final readiness 已标注最终人工文件、WFC Dashboard/HumanApprovalGate fallback，以及数据表动态写回缺口；WFC `ok=true` 运行日志已升级为 live。 | 所有图、视频、指标、日志都有来源；模拟、平台 PoC、真实产线证据分层。 |
...
3. 把剩余 WFC Dashboard 和 HumanApprovalGate fallback 替换成真实平台执行证据；`05-run-log-ok-true.png` 已有 live WFC 原生日志。优先级高于继续润色文案。
