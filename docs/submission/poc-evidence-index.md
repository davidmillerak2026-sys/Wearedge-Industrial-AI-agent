...
| WFC workflow export evidence | `docs/submission/wfc-export-evidence-20260616.md` | generated |
...
- 2026-06-16 live WFC export succeeded for both `File -> Export workflow` (`.wfcw`) and `File -> Export deployment data` (`.wfcd`). These files are archived under ignored `submission-assets/live-evidence/gongyi-mofang/workflow-export/`, but they are proprietary high-entropy binary exports, not readable JSON, so they prove asset export/archival rather than confirmed data-line topology.
