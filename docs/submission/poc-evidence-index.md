...
- Dashboard and HumanApprovalGate still have fallback demo assets under `submission-assets/live-evidence/gongyi-mofang/04*` and `06*`; they are visibly marked or metadata-marked as mock evidence and must not be described as live WFC Dashboard or live approval-node proof. `05-run-log-ok-true.png` is no longer fallback: it is a reviewed live WFC native run-log screenshot showing `CallWearedgeDecisionApi.output` with `"ok": true`. Dynamic Python-output-to-`更新数据表.1` writeback is still open until a clear data-port line, exported workflow binding, or native data-table value proof is captured.
