...
| Jetson edge stdlib gateway latency/resource report | `docs/finals-jetson-gateway-latency-benchmark-report.md` | generated |
| Jetson edge stdlib gateway latency/resource JSON | `docs/submission/evidence/finals-jetson-gateway-latency-benchmark.json` | generated |
...
- Jetson live edge evidence is collected by `scripts/collect_jetson_edge_evidence.py`; the 2026-06-12 run used the isolated remote directory `/home/ryn/Wearedge-Industrial-AI-agent-competition`, produced 300 HTTP decision samples on the Jetson stdlib gateway, and met the <=500 ms target with wall-clock P95 2 ms and max 3 ms. The public report/JSON are stored under `docs/`; the raw `tegrastats` and collection files remain ignored under `submission-assets/live-evidence/edge-runtime/`.
...
- Current WFC integration has real Gongyi Mofang project evidence: authenticated project page, `Wearedge WFC PoC` project, Python function block, data-table fields, `fb_main.py` saved into the live Python block, DEBUG entry, `Workflow is ready` log-manager evidence, 2026-06-12 WFC/SPIDR -> Wearedge gateway `POST /v1/workflow-canvas/decision` `200 OK` auxiliary live-call evidence after the Python Block was connected into the workflow main line, and a later browser-automation status check showing `CallWearedgeDecisionApi` on the WFC outline with `未修改` state.
...
- Dashboard, WFC-native `ok=true` run log, and HumanApprovalGate now have fallback demo assets under `submission-assets/live-evidence/gongyi-mofang/04-06*`; they are visibly marked or metadata-marked as mock/API-smoke evidence and must not be described as live WFC `ok=true` proof until WFC log-manager, inline read output, or data-table writeback visibly shows `ok=true` / latency / decision fields. The 2026-06-12 gateway `POST 200 OK` screenshot proves live WFC/SPIDR invocation of Wearedge, but it is still auxiliary evidence rather than a replacement for the core WFC-native `05-run-log-ok-true.png`.
