...
- Dashboard, WFC-native business `ok=true` output, and HumanApprovalGate now have fallback demo assets under `submission-assets/live-evidence/gongyi-mofang/04-06*`; they are visibly marked or metadata-marked as mock/API-smoke evidence and must not be described as live WFC `ok=true` proof until WFC log-manager, inline read output, or data-table writeback visibly shows `ok=true` / latency / decision fields. The 2026-06-12 gateway `POST 200 OK` screenshot plus `状态码 Good` WFC run-state screenshots prove live WFC/SPIDR execution path, but the current `05-run-log-ok-true.png` remains fallback because it is not a WFC-native business stdout/data-table proof.
