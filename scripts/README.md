...
| [`prepare_wfc_live_edit_package.py`](prepare_wfc_live_edit_package.py) | Prepare a paste-ready WFC Python Function Block package and capture checklist for dynamic data-table writeback evidence. |
...
| [`build_official_attachment_pack.py`](build_official_attachment_pack.py) | Build print-ready official attachment HTML/PDF files and a zip pack from the repo-controlled business plan, technical solution, validation reports, and upload checklists. |
...
| [`promote_wfc_live_evidence.py`](promote_wfc_live_evidence.py) | Promote reviewed live Gongyi Mofang Dashboard/run-log/HumanApprovalGate screenshots from an ignored staging folder, optionally require `.review.json` sidecars, remove fallback markers, and write a replacement manifest. |
...
| [`verify_stable_wearedge_endpoint.py`](verify_stable_wearedge_endpoint.py) | Verify a fixed HTTPS Wearedge endpoint by checking `/healthz`, `/v1/edge/runtime-profile`, and `/v1/workflow-canvas/decision` before using it as reproducible platform evidence. |
...
| [`wfc_private_api_probe.py`](wfc_private_api_probe.py) | Read-only Gongyi Mofang private API probe for project-file, frontend-observed workflow API, legacy workflow JSON, data-table, Dashboard Explorer, and log-manager diagnosis; requires credentials through local environment variables and redacts them from output. |
